...
if ($quizattempts = $DB->get_records('quiz_attempts', array('quiz' => $quizid, 'userid' => $userid))) {
    foreach ($quizattempts as $attempt) {
        $attemptid = $attempt->id;
    } 
} else {
        echo "\I need to create a session. line 42 in iframe.php";
        exit;
}
$quizmodule = $DB->get_record('modules', array('name' => 'quiz'));
$cm = $DB->get_record('course_modules', array('module' => $quizmodule->id, 'instance' => $quizid));
$cmid = $cm->id;
